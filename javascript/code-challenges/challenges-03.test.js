...
const containsAnd = (arr) => {
  let newArr = arr.filter((idx) => idx.includes('and'));
  return newArr;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  ...
  //   const differenceArr = arr.filter((num) => num !== forbiddenValues[i]);
  //   console.log(differenceArr);
  //   returnedArr.push(differenceArr);
  // }
  // return returnedArr;
  let results = arr.filter(item => !forbiddenValues.includes(item));
  return results;
};
