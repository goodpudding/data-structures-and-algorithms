...
const count = (target, input) => {
  let counter = 0;
  input.map((currentArr) =>{
    currentArr.forEach((currentInt) => {
...
let findShortest = (data) => {
  let shorty = '1';
  let shortyName = '';
  data.map((currentCharacter) =>{
    // currentArr.forEach((currentInt) => {
    //   if (currentInt.height > shorty){
    //     shorty = currentInt.
    //   }
    // });
    if(currentCharacter.height > shorty){
      shorty = currentCharacter.height;
      shortyName = currentCharacter.name;
    }
  });
  return shortyName;
};
